...
# Вызов должен быть после определения функции.
...
# Задача 2 (Аргументы функций)
# Напиши функцию multiply_numbers(), которая:
# Принимает два аргумента (числа).
# Возвращает результат их умножения (не выводит!).
# Затем вызови функцию с аргументами 3 и 5 и сохрани результат в переменную result.
# Требования:
# Используй return, а не print().
# Проверь, что функция возвращает правильный результат (например, через print(result)).
...
# Задача 3 (Дефолтные аргументы)
# Напиши функцию make_coffee(), которая:
# Принимает один обязательный аргумент (тип кофе, например "эспрессо").
# И один необязательный аргумент (объём, по умолчанию 200 мл).
# Возвращает строку в формате:
# "Ваш [тип] - [объём] мл готов!"
#
# Требования:
# Необязательный аргумент должен быть на втором месте.
# Используй return, а не print().

def make_coffee(coffee, volume=200):
    return f"Ваш {coffee} - {volume} мл готов!"

print(make_coffee("латте"))
print(make_coffee("капучино", 300))

# Задача 4 (Возвращение данных)
# Напиши функцию calculate(), которая:
# Принимает два числа и строку (операция: "+", "-", "*", "/").
# Возвращает результат этой операции (число).
# Если операция не распознана — возвращает None.
# Требования:
# Используй if-elif-else для проверки операций.
# Не обрабатывай деление на ноль (пока).
...
# Задача 5 (Декораторы)
# Напиши декоратор add_hello, который:
# Добавляет строку "Hello, " перед результатом любой декорируемой функции.
# Сама функция должна возвращать строку (для простоты).
# Требования:
# Декоратор должен принимать функцию как аргумент.
# Используй вложенную функцию-обёртку.
# Не изменяй оригинальную функцию greet.
...
# Задача 6 (Возвращение нескольких значений)
# Напиши функцию get_circle_info(), которая:
# Принимает радиус окружности.
# Возвращает два значения:
# Длину окружности (формула: 2 * 3.14 * радиус).
# Площадь круга (формула: 3.14 * радиус²).
# Требования:
# Используй return с двумя значениями через запятую.
# Для возведения в квадрат примени radius ** 2.

def get_circle_info(r, Pi=3.14):
    length = 2 * Pi * r
    area = Pi * r ** 2
    return length, area

length, area = get_circle_info(5)
print(length)
print(area)

# Задача 7 (Рекурсия)
# Напиши функцию countdown(n), которая:
# Принимает целое число n.
# Рекурсивно выводит числа от n до 1, а затем "Пуск!".
# При n <= 0 сразу возвращает "Пуск!".
# Пример вызова:
# countdown(3)
# Вывод в консоль:
# 3
...
# Пуск!
# Требования:
# Используй print() внутри функции.
# Базовый случай: n <= 0.
